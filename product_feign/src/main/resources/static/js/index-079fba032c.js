            ...
            success: function (data) {
                ...
                for (var k = 0; k < data.length; k++) {
                    ...
                    if (data[k].type == '蛋壳故事') {
                        ...
                        for (var i = 0; i < data[k].data.length; i++) {
                            story += '<a key="' + i + 1 + '" href="javascript:void(0)" data-lang="' + data[k].data[i].url + '" class="storylist s-l' + parseInt(i + 1) + '">' +
                                '<img title="' + data[k].data[i].image_title + '" src="' + data[k].data[i].image + '" alt="蛋壳故事">' +
                                '<div class="sl-text">' +
                                '<em class="dankegongyucomicp">付费文章</em>' +
                                '<label>了解更多</label>' +
                        ...
                        if (loc.split('www.dankegongyu.com').length > 1) {
                            $('.dankegongyucomicp').show();
                        }
                        $('.storybox a').click(function () {
                            if (loc.split('www.dankegongyu.com').length > 1) {
                                if ($(this).hasClass('on')) {
                                    window.open($(this).attr('data-lang'));
                                } else {
                                    $(this).addClass('on');
                                    $('.website-add-paycode').show();
                                }
                            } else {
                                window.location.href = $(this).attr('data-lang');
                            }
                        });
                        $('.website-add-paycode').click(function () {
                            $('.website-add-paycode').hide();
                        })
                    }
